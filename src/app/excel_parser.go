...
func Run(sourceDir string, outDir string, outFile string, binary bool, outDefDir string, outDefFile string, defNamespace string) error {
	initData()
	log.Println("run.args: ", sourceDir, outDir, outDefDir)
	suffix := ".xlsx|.xls"
	//sourceDir := "./assets/"
	//outDir := "./configs/"
	//outDefDir := "./def/"
	...
	files := GetEffectiveFiles(sourceDir, pattern)
	numFiles := len(files)
	...
	if numFiles > 0 {
		...
		if bError.Len() > 0 {
			fmt.Printf("\n错误：\n%s", bError.String())
			return errors.New(bError.String())
		} else {
			fDTS, bDTS := GetFillAndBufferWriter(fmt.Sprintf("%s\\%s.d.ts", outDefDir, outDefFile))
			defer fDTS.Close()
			...
			fConfig, bConfig := GetFillAndBufferWriter(fmt.Sprintf("%s\\%s%s", outDir, outFile, outSuffix))
			defer fConfig.Close()
			if binary {
				encoder := gob.NewEncoder(bConfig)
				_ = encoder.Encode(bData)
			} else {
				for i, b := range bData.Bytes() {
					if b < 128 {
						bData.Bytes()[i] = b + 1
					}
				}
...
func parseSheetOfObject(rows [][]string, sheet string) (*ParseResult, error) {
	var bDefine, bData, bError bytes.Buffer
	var interfaceName = StringToCamel(sheet, true)
	var attrName = StringToCamel(sheet, false)
	var tab = &Tab{Content: ""}
	tab.Indent(1)
	bDefine.WriteString(tab.Content + "interface I" + interfaceName + " {")
	tab.Indent(1)
	bData.WriteString(attrName + " object\n")
	...
	for i, row := range rows {
		...
		fieldName := StringToCamel(row[2], false)
		bDefine.WriteString(tab.Content + "readonly " + fieldName + ": " + getTSType(row[1]) + ";")

		//数据
		bData.WriteString(getTSType(row[1]) + " " + row[2] + " " + encode(row[3]) + "\n")
	}
	tab.Indent(-1)
...
func parseSheetOfObjectArray(rows [][]string, sheet string) (*ParseResult, error) {
	...
	for i := 0; i < numCol; i++ {
		...
		fieldName := StringToCamel(rows[2][i], false)
	...
	/** 数据 */
	bData.WriteString(attrName + " object_array\n")
	//写入类型
	for i := 0; i < numCol; i++ {
	...
	//写入字段名
	for i := 0; i < numCol; i++ {
		bData.WriteString(StringToCamel(rows[2][i], false))
		if i != numCol-1 {
			bData.WriteString(" ")
		} else {
			bData.WriteString("\n")
